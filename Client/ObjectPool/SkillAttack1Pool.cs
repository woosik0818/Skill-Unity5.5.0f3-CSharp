...
public class SkillAttack1Pool : ObjectPool 
{	
    // 싱글톤(Singleton) 형식으로 타격 이펙트1을 생성하는 오브젝트 풀을 생성합니다.
	private static SkillAttack1Pool _instance;
	public static SkillAttack1Pool Instance
	{
		get
		{
            ...
            if (!_instance)
            {
                _instance = GameObject.FindObjectOfType(typeof(SkillAttack1Pool)) as SkillAttack1Pool;
                if (!_instance)
                {
                    GameObject container = new GameObject();
                    _instance = container.AddComponent(typeof(SkillAttack1Pool)) as SkillAttack1Pool;
                }
            }		
			return _instance;
		}
	...
	public void Init()
	{
		...
		prefabName = "Prefab/SkillAttack1";
        // 초기에 만들 오브젝트 수를 정합니다.
		poolSize = 20;
